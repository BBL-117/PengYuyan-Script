...
local function updatePlayerCard(player)
    ...
    if not leaderstats then
        cardData.paragraph:SetDesc("玩家数据未加载")
        return
    end
    ...
    if not strength or not rebirths then
        cardData.paragraph:SetDesc("缺少属性 (Strength/Rebirths)")
        return
    end
    ...
    cardData.paragraph:SetDesc(desc)
...
local function refreshAllPlayers()
    ...
    for userId, cardData in pairs(playerCards) do
        if not currentIds[userId] then
            if cardData.paragraph.Visible ~= nil then
                cardData.paragraph.Visible = false
            end
        else
            if cardData.paragraph.Visible ~= nil then
                cardData.paragraph.Visible = true
            end
        end
    end
