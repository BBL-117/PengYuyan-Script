...
local weatherRoot = nil
local weatherConn = nil
local currentWeather = "无"
local orig = {
...
local function resetLighting()
    for k, v in pairs(orig) do pcall(function() Lighting[k] = v end) end
end

local function clearWeather()
    if weatherConn then weatherConn:Disconnect() weatherConn = nil end
    if weatherRoot then weatherRoot:Destroy() weatherRoot = nil end
    resetLighting()
end

local function makeEmitter(cfg)
    ...
    p.Parent = weatherRoot
    local e = Instance.new("ParticleEmitter")
    e.Parent = p
    for k,v in pairs(cfg) do pcall(function() e[k]=v end) end
    if not weatherConn then
        weatherConn = RunService.RenderStepped:Connect(function()
            local cam = workspace.CurrentCamera
            if cam and p then p.CFrame = cam.CFrame end
        end)
    end
    return e
end

local function setWeather(opt)
    clearWeather()
    currentWeather = opt
    if opt == "无" then return end
    weatherRoot = Instance.new("Folder")
    weatherRoot.Name = "WeatherFX"
    weatherRoot.Parent = workspace

    if opt == "阴雨天" then
        Lighting.Brightness = 1.5
        Lighting.Ambient = Color3.new(0.2,0.25,0.3)
        Lighting.ColorShift_Top = Color3.new(0.3,0.3,0.5)
        Lighting.OutdoorAmbient = Color3.new(0.4,0.5,0.6)
        makeEmitter({
            Texture = "rbxasset://textures/particles/sparkles_main.dds",
            Color = ColorSequence.new(Color3.new(0.7,0.8,1)),
            Size = NumberSequence.new({{0.1,0.1},{0.2,0.4}}),
            ...
            Transparency = NumberSequence.new({{0.2,0.2},{0.6,0.6},{1,1}}),
        })
    elseif opt == "暴雪天" then
        Lighting.Brightness = 1.2
        Lighting.Ambient = Color3.new(0.5,0.6,0.7)
        Lighting.ColorShift_Top = Color3.new(0.6,0.7,0.9)
        Lighting.OutdoorAmbient = Color3.new(0.5,0.6,0.8)
        makeEmitter({
            Texture = "rbxasset://textures/particles/sparkles_main.dds",
            Color = ColorSequence.new(Color3.new(0.9,0.95,1)),
            Size = NumberSequence.new({{0.1,0.1},{0.25,0.3}}),
            Lifetime = NumberRange.new(0.6,1),
            Rate = 1200,
            Speed = NumberRange.new(10,18),
            Direction = Vector3.new(0,-1,0),
            SpreadAngle = Vector2.new(15,15),
            Acceleration = Vector3.new(3,-15,2),
            Transparency = NumberSequence.new({{0.1,0.1},{0.7,0.7},{1,1}}),
        })
    elseif opt == "极光天" then
        Lighting.Brightness = 0.8
        Lighting.Ambient = Color3.new(0.1,0.2,0.3)
        Lighting.ColorShift_Top = Color3.new(0.8,0.9,1)
        Lighting.ColorShift_Bottom = Color3.new(0.2,0.6,0.8)
        Lighting.OutdoorAmbient = Color3.new(0.3,0.5,0.7)
    elseif opt == "漫天星辰天" then
        ...
        for i=1,200 do
            ...
            s.Transparency = 0.3+math.random()*0.5
            local theta = math.random()*2*math.pi
            local phi = math.random()*math.pi/2
            local dist = 500+math.random()*300
            s.Position = Vector3.new(math.cos(theta)*math.cos(phi)*dist, math.sin(phi)*dist+100, math.sin(theta)*math.cos(phi)*dist)
            s.Parent = weatherRoot
            task.spawn(function()
                while s and s.Parent do
                    s.Transparency = 0.3+math.random()*0.6
                    task.wait(0.5+math.random())
                end
            end)
        end
        task.spawn(function()
            while currentWeather == "漫天星辰天" do
                ...
                m.Size = Vector3.new(0.6,0.6,0.6)
                m.Material = Enum.Material.Neon
                m.Color = Color3.new(0.4,0.6,1)
                local start = Vector3.new(math.random(-800,800), 200+math.random(0,200), math.random(-800,800))
                local dir = Vector3.new(math.random(-30,30), -50-math.random(0,30), math.random(-30,30))
                m.Position = start
                m.Parent = weatherRoot
                local trail = Instance.new("Trail")
                trail.Parent = m
                trail.Lifetime = 0.5
                trail.MinLength = 0.1
                trail.Color = ColorSequence.new(Color3.new(0.6,0.8,1))
                trail.Transparency = NumberSequence.new({{0,0},{0.5,0.5},{1,1}})
                task.spawn(function()
                    for t=0, 2, 0.02 do
                        if not m.Parent then break end
                        m.Position = start + dir * t
                        task.wait(0.02)
                    end
                    m:Destroy()
                end)
    ...
    elseif opt == "太阳天" then
        Lighting.Brightness = 9999999999
        ...
        for i=1,10 do
            ...
            sun.Color = Color3.new(1,0.8,0)
            sun.Transparency = 0.2
            local ang = (i-1)/10 * 2*math.pi
            sun.Position = Vector3.new(500*math.cos(ang), 400, 500*math.sin(ang))
            sun.Parent = weatherRoot
        end
    elseif opt == "纷飞雪天" then
        Lighting.Brightness = 1.8
        Lighting.Ambient = Color3.new(0.6,0.7,0.8)
        Lighting.OutdoorAmbient = Color3.new(0.6,0.7,0.9)
        makeEmitter({
            Texture = "rbxasset://textures/particles/sparkles_main.dds",
            Color = ColorSequence.new(Color3.new(1,1,1)),
            Size = NumberSequence.new({{0.15,0.15},{0.3,0.3}}),
            ...
            Transparency = NumberSequence.new({{0.1,0.1},{0.5,0.5},{1,1}}),
        })
    elseif opt == "月光天" then
        Lighting.Brightness = 0.2
        Lighting.Ambient = Color3.new(0.05,0.05,0.15)
        Lighting.ColorShift_Top = Color3.new(0.1,0.1,0.3)
        Lighting.OutdoorAmbient = Color3.new(0.05,0.05,0.2)
        ...
        moon.Color = Color3.new(0.8,0.8,1)
        moon.Transparency = 0.2
        moon.Position = Vector3.new(0, 350, -500)
        moon.Parent = weatherRoot
        for i=1,100 do
            local s = Instance.new("Part")
            s.Name = "Star"
            s.Anchored = true
            s.CanCollide = false
            s.Shape = Enum.PartType.Ball
            s.Size = Vector3.new(0.15,0.15,0.15)
            s.Material = Enum.Material.Neon
            s.Color = Color3.new(1,1,1)
            s.Transparency = 0.3+math.random()*0.5
            local theta = math.random()*2*math.pi
            local phi = math.random()*math.pi/2
            local dist = 400+math.random()*200
            s.Position = Vector3.new(math.cos(theta)*math.cos(phi)*dist, math.sin(phi)*dist+80, math.sin(theta)*math.cos(phi)*dist)
            s.Parent = weatherRoot
        end
    end
end

local WeatherGroup = Tabs.Skins:AddLeftGroupbox("天气场景", "cloud-sun")
WeatherGroup:AddDropdown("WeatherSelect", {
    Text = "选择天气",
    Values = {"无","阴雨天","暴雪天","极光天","漫天星辰天","太阳天","纷飞雪天","月光天"},
    Default = "无",
    Callback = function(v) setWeather(v) end
})
