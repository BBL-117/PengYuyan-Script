...
local function IsVisible(p)
    ...
    if r then
        return r.Instance:IsDescendantOf(p.Parent)
    end
    return true
end
local hotkeyValues = {
...
local function getHotkeyName(k)
    for n, v in pairs(hotkeyMap) do
        if v == k then
            return n
        end
    end
...
local function isEnemy(plr)
    ...
    if not mc or not tc or not mc.Parent or not tc.Parent then
        return false
    end
...
local function getClosestEnemy()
    local cl = nil
    local sd = VisualAimbot.Radius
    local ef = getEnemyFolder()
    if not ef or not VisualAimbot.Enabled then
        return nil
    end
    ...
    for _, e in ipairs(ef:GetChildren()) do
        ...
        if hum and hum.Health > 0 and hd then
            if VisualAimbot.WallCheck and not IsVisible(hd) then
                continue
            end
...
UserInputService.InputBegan:Connect(function(i)
    if i.UserInputType == VisualAimbot.Key or i.KeyCode == VisualAimbot.Key then
        VisualAimbot.KeyHeld = true
    end
end)
UserInputService.InputEnded:Connect(function(i)
    if i.UserInputType == VisualAimbot.Key or i.KeyCode == VisualAimbot.Key then
        VisualAimbot.KeyHeld = false
    end
end)
RunService.RenderStepped:Connect(function()
    ...
    if not VisualAimbot.Enabled or not isAlive() then
        return
    end
...
UserInputService.InputBegan:Connect(function(i)
    if i.UserInputType == TriggerBot.Key or i.KeyCode == TriggerBot.Key then
        TriggerBot.KeyHeld = true
    end
end)
UserInputService.InputEnded:Connect(function(i)
    if i.UserInputType == TriggerBot.Key or i.KeyCode == TriggerBot.Key then
        TriggerBot.KeyHeld = false
    end
end)
task.spawn(function()
    while task.wait(0.01) do
        ...
        if TriggerBot.Enabled and isAlive() then
            if TriggerBot.Mode == "自动" then
                shoot = true
            elseif TriggerBot.Mode == "热键" then
                shoot = TriggerBot.KeyHeld
            end
        end
        if not shoot then
            continue
            ...
            if model and model:FindFirstChildOfClass("Humanoid") then
                ...
                if ef and model.Parent == ef then
                    ...
                    if hum and hum.Health > 0 then
                        if TriggerBot.WallCheck and not IsVisible(model:FindFirstChild("Head")) then
                            continue
                        end
                        if TriggerBot.Delay > 0 then
                            task.wait(TriggerBot.Delay / 1000)
                        end
                        if mouse1click then
                            mouse1click()
                        end
        ...
        if ef then
            for _, e in ipairs(ef:GetChildren()) do
                ...
                if hd and hm and hm.Health > 0 then
                    if not originalHeadSizes[hd] then
                        originalHeadSizes[hd] = hd.Size
                    end
                    if Hitbox.Enabled then
                        ...
                        hd.Size = originalHeadSizes[hd] or Vector3.new(2, 2, 1)
                        hd.Transparency = 0
                    end
                end
            end
        ...
        if root then
            ...
            if horizontalVel.Magnitude > 0.1 then
                ...
                root.AssemblyLinearVelocity = Vector3.new(
                    direction.X * (16 + SpeedHack.Value * 50),
                    vel.Y,
                    direction.Z * (16 + SpeedHack.Value * 50)
                )
            end
        end
    end
...
AimbotSection:Dropdown({
    ...
    Multi = false,
    Callback = function(v)
        VisualAimbot.Mode = v[1]
    end
})
AimbotSection:Dropdown({
    Title = "自瞄热键",
    Values = hotkeyValues,
    Value = {getHotkeyName(VisualAimbot.Key)},
    Multi = false,
    Callback = function(v)
        VisualAimbot.Key = hotkeyMap[v[1]] or Enum.UserInputType.MouseButton2
    end
})
AimbotSection:Toggle({
    Title = "墙壁检测",
...
TriggerSection:Dropdown({
    Title = "扳机热键",
    Values = hotkeyValues,
    Value = {getHotkeyName(TriggerBot.Key)},
    Multi = false,
    Callback = function(v)
        TriggerBot.Key = hotkeyMap[v[1]] or Enum.KeyCode.E
    end
})
TriggerSection:Toggle({
    Title = "墙壁检测",
...
local MovementSection = CombatTab:Section({
    Title = "移动"
})
...
local function updateTargetList()
    for _, toggle in pairs(targetToggles) do
        toggle:Destroy()
    end
...
local function getWallbangTarget(hrp, partName)
    ...
    if Wallbang.TargetMode == "全部随机" then
        ...
        for _, e in ipairs(enemies) do
            if SelectedTargets[e.model.Name] then
...
local function updateWallWeapon()
    for _, v in next, getgc(true) do
        if type(v) == "table" and rawget(v, "IsEquipped") and rawget(v, "Identifier") and rawget(v, "Player") == player then
            if isWallbangWeapon(v) then
                wallCurrentWeapon = v
                return true
            end
        end
    end
    wallCurrentWeapon = nil
    return false
end
...
local function wallbangShoot()
    ...
    if tick() - wallLastShot < Wallbang.Delay then
        return
    end
    local char = player.Character
    if not char or char:GetAttribute("Dead") then
        return
    end
    ...
    if not wallCurrentWeapon or not rawget(wallCurrentWeapon, "IsEquipped") then
        if not updateWallWeapon() then
            return
        end
    end
    if not isWallbangWeapon(wallCurrentWeapon) then
        wallCurrentWeapon = nil
        return
    end
    local prop = rawget(wallCurrentWeapon, "Properties")
    local max = rawget(prop, "Rounds")
    local cur = rawget(wallCurrentWeapon, "Rounds")
    if not (max and cur) then
        return
    end
    if cur <= 0 then
    ...
    if not remote or not remote.ShootWeapon then
        return
    end
    ...
    remote.ShootWeapon.Send({
        ...
        ShootingHand = "Right",
        Identifier = wallCurrentWeapon.Identifier,
        Capacity = wallCurrentWeapon.Capacity,
        Rounds = wallCurrentWeapon.Rounds,
        Bullets = {{
            Direction = dir,
            Origin = origin,
            Hits = {{
                ...
                Normal = -dir,
                Material = "Plastic",
                Distance = (target.part.Position - origin).Magnitude,
                Exit = false
            }}
        }}
    })
    ...
    task.delay(0.3, function()
        local isDead = false
        if targetHum then
            if targetHum.Health <= 0 or not targetHum.Parent then
                isDead = true
            end
        else
            isDead = true
        ...
        if isDead then
            ...
            if Wallbang.SoundEnabled then
                playSoundSafe(Wallbang.SoundID)
            end
        end
    end)
end
UserInputService.InputBegan:Connect(function(i)
    if i.UserInputType == Wallbang.Key or i.KeyCode == Wallbang.Key then
        Wallbang.KeyHeld = true
    end
end)
UserInputService.InputEnded:Connect(function(i)
    if i.UserInputType == Wallbang.Key or i.KeyCode == Wallbang.Key then
        Wallbang.KeyHeld = false
    end
end)
RunService.Heartbeat:Connect(function()
    ...
    local shoot = false
    if Wallbang.Mode == "自动" then
        shoot = true
    elseif Wallbang.Mode == "热键" then
        shoot = Wallbang.KeyHeld
    end
    if shoot and isAlive() then
        wallbangShoot()
    end
end)
local WallbangSection = CombatTab:Section({
    Title = "静默穿墙"
})
...
WallbangSection:Dropdown({
    Title = "热键",
    Values = hotkeyValues,
    Value = {getHotkeyName(Wallbang.Key)},
    Multi = false,
    Callback = function(v)
        Wallbang.Key = hotkeyMap[v[1]] or Enum.KeyCode.F
    end
})
WallbangSection:Slider({
    Title = "射击间隔(秒)",
    Step = 0.1,
    Value = {
        Min = 0.2,
...
WallbangSection:Dropdown({
    Title = "击打部位",
    Values = {"头部", "身体", "左腿", "右腿", "左臂", "右臂"},
    Value = {"身体"},
    Multi = false,
    Callback = function(v)
        Wallbang.HitPart = v[1]
    end
})
WallbangSection:Dropdown({
    Title = "目标模式",
    ...
    Multi = false,
    Callback = function(v)
        Wallbang.TargetMode = v[1]
        TargetFrame.Visible = (v[1] == "指定玩家")
        if v[1] == "指定玩家" then
            updateTargetList()
        end
    end
...
WallbangSection:Slider({
    ...
    Callback = function(v)
        bgR = v
        HitNotif.BgColor = Color3.fromRGB(bgR, bgG, bgB)
    end
})
WallbangSection:Slider({
    Title = "绿",
...
WallbangSection:Slider({
    ...
    Callback = function(v)
        textR = v
        HitNotif.TextColor = Color3.fromRGB(textR, textG, textB)
    end
})
WallbangSection:Slider({
    Title = "绿",
...
WallbangSection:Slider({
    ...
    Callback = function(v)
        deathR = v
        HitNotif.DeathColor = Color3.fromRGB(deathR, deathG, deathB)
    end
})
WallbangSection:Slider({
    Title = "绿",
...
local Config = {
    ESP = {
        ...
        BoxOutline = false,
        BoxFill = false,
        BoxFillColor1 = Color3.fromRGB(255, 0, 0),
        BoxFillColor2 = Color3.fromRGB(0, 0, 255),
...
local function espIsVisible(originChar, targetChar)
    if not originChar or not targetChar or not targetChar.Parent then
        return true
    end
    local origin = originChar:FindFirstChild("Head") or originChar.PrimaryPart or originChar:FindFirstChild("HumanoidRootPart")
    if not origin then
        return true
    end
    pcall(function()
        espRayParams.FilterDescendantsInstances = {originChar, targetChar}
    end)
...
local function createPlayerESP()
    ...
    for i = 1, 4 do
        e.BoxOutline[i] = newDrawing("Line", {
            Thickness = 3,
            Color = Color3.new(0, 0, 0),
            Visible = false,
            ZIndex = 1
        })
        e.Box[i] = newDrawing("Line", {
            Thickness = 1,
            Visible = false,
            ZIndex = 2
        })
    ...
    for i = 1, 20 do
        e.Skeleton[i] = newDrawing("Line", {
            Thickness = 2,
            Visible = false
        })
    end
    e.HeadDot = newDrawing("Circle", {
        Thickness = 1,
        NumSides = 30,
        Filled = false,
        Visible = false
    })
    e.HpBg = newDrawing("Line", {
        Thickness = 2,
        Visible = false,
        Color = Color3.new(0, 0, 0),
        Transparency = 0.5,
    ...
    e.Hp = newDrawing("Line", {
        Thickness = 2,
        Visible = false,
        ZIndex = 3
    })
    e.Name = newDrawing("Text", {
        Size = 13,
        Center = true,
        Outline = true,
        Font = 2,
...
local function hidePlayerESP(e)
    ...
    for _, d in pairs(e.Box) do
        if d then
            d.Visible = false
        end
    end
    for _, d in pairs(e.BoxOutline) do
        if d then
            d.Visible = false
        end
    end
    for _, d in pairs(e.Skeleton) do
        if d then
            d.Visible = false
        end
    end
    for _, d in pairs(e.Fill) do
        pcall(function()
            d.Visible = false
        end)
    end
...
local function destroyPlayerESP(e)
    ...
    for _, list in pairs({e.Box, e.BoxOutline, e.Skeleton, e.Fill}) do
...
local function hideWorldESPObj(e)
    if not e then
        return
    end
    for _, d in pairs(e.Box) do
    ...
    if e.Radius then
        e.Radius.Visible = false
    end
    if e.HL then
        e.HL.Enabled = false
    end
end
local function destroyWorldESPObj(e)
    hideWorldESPObj(e)
    if not e then
        return
    end
...
RunService.RenderStepped:Connect(function()
    ...
    if not localHuman or localHuman.Health <= 0 then
        return
    end
    ...
    if Config.ESP.Enabled and now - lastESPUpdate > 0.015 then
        ...
        for p, e in pairs(playerESP) do
            ...
            if Config.ESP.TeamCheck and not isEnemy(p) then
                hidePlayerESP(e)
                continue
            end
            ...
            local tl = Vector2.new(cx - sW / 2, topY)
            local tr = Vector2.new(cx + sW / 2, topY)
            local bl = Vector2.new(cx - sW / 2, botY)
            local br = Vector2.new(cx + sW / 2, botY)
            if Config.ESP.Box then
                ...
                for i = 1, 4 do
                    if e.Box[i] then
                        e.Box[i].Thickness = Config.ESP.BoxThickness
                    end
                end
                if e.Box[1] then
                    e.Box[1].From = tl
                    e.Box[1].To = tr
                    e.Box[1].Color = col
                    e.Box[1].Visible = true
                end
                if e.Box[2] then
                    e.Box[2].From = tr
                    e.Box[2].To = br
                    e.Box[2].Color = col
                    e.Box[2].Visible = true
                end
                if e.Box[3] then
                    e.Box[3].From = br
                    e.Box[3].To = bl
                    e.Box[3].Color = col
                    e.Box[3].Visible = true
                end
                if e.Box[4] then
                    e.Box[4].From = bl
                    e.Box[4].To = tl
                    e.Box[4].Color = col
                    e.Box[4].Visible = true
                end
                if Config.ESP.BoxOutline then
                    for i = 1, 4 do
                        if e.BoxOutline[i] and e.Box[i] then
                    ...
                    for i = 1, 4 do
                        if e.BoxOutline[i] then
                            e.BoxOutline[i].Visible = false
                        end
                    end
                end
            else
            ...
            if Config.ESP.BoxFill then
                if e.Fill[1] and type(e.Fill[1]) ~= "number" then
                    e.Fill[1].PointA = tl
                    e.Fill[1].PointB = tr
                    e.Fill[1].PointC = bl
                ...
                if e.Fill[2] and type(e.Fill[2]) ~= "number" then
                    e.Fill[2].PointA = tr
                    e.Fill[2].PointB = br
                    e.Fill[2].PointC = bl
                ...
                for i = 1, 2 do
                    if e.Fill[i] and type(e.Fill[i]) ~= "number" then
                        e.Fill[i].Visible = false
                    end
                end
            end
            local tY = tl.Y - 18
            ...
            if Config.ESP.Health and hum then
                ...
                local barH = sH * hpF
                if e.HpBg then
                    e.HpBg.From = Vector2.new(bx, bl.Y)
                    e.HpBg.To = Vector2.new(bx, tl.Y)
                    e.HpBg.Visible = true
                end
                if e.Hp then
                    e.Hp.From = Vector2.new(bx, bl.Y)
                    e.Hp.To = Vector2.new(bx, bl.Y - barH)
            ...
            if Config.ESP.Skeleton and hum and dist < 300 then
                ...
                for _, ln in ipairs(e.Skeleton) do
                    if ln then
                        ln.Visible = false
                    end
                end
                ...
                if Config.WorldESP.Bomb.Enabled and item:IsA("Model") and item.Name == "Character" and item:GetAttribute("BombPlanted") then
                    ...
                    if not worldESP.Bomb or worldESP.Bomb.Model ~= item then
                        if worldESP.Bomb then
                            destroyWorldESPObj(worldESP.Bomb)
                        end
    ...
    if camera then
        ...
        for item, eo in pairs(worldESP.DroppedWeapons) do
            ...
            if on and pos.Z > 0 then
                if Config.WorldESP.DroppedWeapons.Box then
                    local tl2 = Vector2.new(pos.X - 15, pos.Y - 10)
                    local tr2 = Vector2.new(pos.X + 15, pos.Y - 10)
                    local bl2 = Vector2.new(pos.X - 15, pos.Y + 10)
                    local br2 = Vector2.new(pos.X + 15, pos.Y + 10)
                    if eo.Box[1] then
                        eo.Box[1].From = tl2
                        eo.Box[1].To = tr2
                        eo.Box[1].Color = Config.WorldESP.DroppedWeapons.Color
                    end
                    if eo.Box[2] then
                        eo.Box[2].From = tr2
                        eo.Box[2].To = br2
                        eo.Box[2].Color = Config.WorldESP.DroppedWeapons.Color
                    end
                    if eo.Box[3] then
                        eo.Box[3].From = br2
                        eo.Box[3].To = bl2
                        eo.Box[3].Color = Config.WorldESP.DroppedWeapons.Color
                    end
                    if eo.Box[4] then
                        eo.Box[4].From = bl2
                        eo.Box[4].To = tl2
                        eo.Box[4].Color = Config.WorldESP.DroppedWeapons.Color
                    end
                    for i = 1, 4 do
                        eo.Box[i].Visible = true
                    end
                else
                    for i = 1, 4 do
                        eo.Box[i].Visible = false
                    end
                end
                if eo.Name and Config.WorldESP.DroppedWeapons.Name then
                    eo.Name.Text = item:GetAttribute("Weapon") or "武器"
                    eo.Name.Position = Vector2.new(pos.X, pos.Y - 25)
                    eo.Name.Color = Config.WorldESP.DroppedWeapons.Color
                    eo.Name.Visible = true
                elseif eo.Name then
                    eo.Name.Visible = false
                end
                if eo.HL then
                    eo.HL.Adornee = item
                    eo.HL.FillColor = Config.WorldESP.DroppedWeapons.Color
                    eo.HL.OutlineColor = Color3.new(1, 1, 1)
                    eo.HL.Enabled = Config.WorldESP.DroppedWeapons.Highlight
                end
            else
                hideWorldESPObj(eo)
            end
        ...
        if worldESP.Bomb and worldESP.Bomb.Model and worldESP.Bomb.Model.PrimaryPart then
            ...
            if on and pos.Z > 0 then
                ...
                local cx2 = pos.X
                local cy2 = (topS.Y + botS.Y) / 2
                local tl3 = Vector2.new(cx2 - sW / 2, cy2 - sH / 2)
                local tr3 = Vector2.new(cx2 + sW / 2, cy2 - sH / 2)
                local bl3 = Vector2.new(cx2 - sW / 2, cy2 + sH / 2)
                local br3 = Vector2.new(cx2 + sW / 2, cy2 + sH / 2)
                if Config.WorldESP.Bomb.Box then
                    if worldESP.Bomb.Box[1] then
                        worldESP.Bomb.Box[1].From = tl3
                        worldESP.Bomb.Box[1].To = tr3
                        worldESP.Bomb.Box[1].Color = Config.WorldESP.Bomb.Color
                    end
                    if worldESP.Bomb.Box[2] then
                        worldESP.Bomb.Box[2].From = tr3
                        worldESP.Bomb.Box[2].To = br3
                        worldESP.Bomb.Box[2].Color = Config.WorldESP.Bomb.Color
                    end
                    if worldESP.Bomb.Box[3] then
                        worldESP.Bomb.Box[3].From = br3
                        worldESP.Bomb.Box[3].To = bl3
                        worldESP.Bomb.Box[3].Color = Config.WorldESP.Bomb.Color
                    end
                    if worldESP.Bomb.Box[4] then
                        worldESP.Bomb.Box[4].From = bl3
                        worldESP.Bomb.Box[4].To = tl3
                        worldESP.Bomb.Box[4].Color = Config.WorldESP.Bomb.Color
                    end
                    for i = 1, 4 do
                        worldESP.Bomb.Box[i].Visible = true
                    end
                else
                    for i = 1, 4 do
                        worldESP.Bomb.Box[i].Visible = false
                    end
                end
                if worldESP.Bomb.Name and Config.WorldESP.Bomb.Name then
                    worldESP.Bomb.Name.Text = "C4"
                    worldESP.Bomb.Name.Position = Vector2.new(cx2, topS.Y - 15)
                    worldESP.Bomb.Name.Color = Config.WorldESP.Bomb.Color
                    worldESP.Bomb.Name.Visible = true
                elseif worldESP.Bomb.Name then
                    worldESP.Bomb.Name.Visible = false
                end
                if worldESP.Bomb.HL then
                    worldESP.Bomb.HL.Adornee = item
                    worldESP.Bomb.HL.FillColor = Config.WorldESP.Bomb.Color
                    worldESP.Bomb.HL.OutlineColor = Color3.new(1, 1, 1)
                    worldESP.Bomb.HL.Enabled = Config.WorldESP.Bomb.Highlight
                end
            else
                hideWorldESPObj(worldESP.Bomb)
            end
        end
        for item, eo in pairs(worldESP.Molotovs) do
            ...
            if center and radius > 0 then
                ...
                if on and pos.Z > 0 then
                    ...
                    if eo.Radius then
                        eo.Radius.Position = Vector2.new(pos.X, pos.Y)
                        eo.Radius.Radius = screenRadius
                        eo.Radius.Color = Config.WorldESP.Molotovs.Color
                        eo.Radius.Visible = true
                    end
                    if eo.Name then
                        eo.Name.Text = "火焰"
                        eo.Name.Position = Vector2.new(pos.X, pos.Y - 15)
                        eo.Name.Color = Config.WorldESP.Molotovs.Color
                        eo.Name.Visible = true
                    end
                else
                    if eo.Radius then
                        eo.Radius.Visible = false
                    end
                    if eo.Name then
                        eo.Name.Visible = false
                    end
            ...
            if eo.HL then
                ...
                if fc then
                    eo.HL.Adornee = fc
                    eo.HL.FillColor = Config.WorldESP.Molotovs.Color
                    eo.HL.OutlineColor = Color3.new(1, 1, 1)
                    eo.HL.Enabled = Config.WorldESP.Molotovs.Highlight
                else
                    eo.HL.Enabled = false
                end
            end
        ...
        for item, eo in pairs(worldESP.Smokes) do
            ...
            if center and radius > 0 then
                ...
                if on and pos.Z > 0 then
                    ...
                    if eo.Radius then
                        eo.Radius.Position = Vector2.new(pos.X, pos.Y)
                        eo.Radius.Radius = screenRadius
                        eo.Radius.Color = Config.WorldESP.Smokes.Color
                        eo.Radius.Visible = true
                    end
                    if eo.Name then
                        eo.Name.Text = "烟雾"
                        eo.Name.Position = Vector2.new(pos.X, pos.Y - 15)
                        eo.Name.Color = Config.WorldESP.Smokes.Color
                        eo.Name.Visible = true
                    end
                else
                    if eo.Radius then
            ...
            if eo.HL then
                ...
                if fc then
                    eo.HL.Adornee = fc
                    eo.HL.FillColor = Config.WorldESP.Smokes.Color
                    eo.HL.OutlineColor = Color3.new(1, 1, 1)
                    eo.HL.Enabled = Config.WorldESP.Smokes.Highlight
                else
                    eo.HL.Enabled = false
                end
            end
    ...
    if Config.Charms.Enabled then
        if not charmFolder then
            charmFolder = Instance.new("Folder", CoreGui)
            charmFolder.Name = "Charms_Container"
        end
        if now - lastCharmUpdate > 0.3 then
            lastCharmUpdate = now
            local scanNeeded = (now - (charmVisCache._lastScan or 0)) > 1
            if scanNeeded then
                charmVisCache._lastScan = now
            end
            for _, plr in ipairs(Players:GetPlayers()) do
                if plr ~= player then
                    ...
                    if char and char:FindFirstChild("HumanoidRootPart") and hum and hum.Health > 0 then
                        if Config.Charms.TeamCheck and not isEnemy(plr) then
                            if charmCache[plr] then
                                for _, box in pairs(charmCache[plr]) do
                                    box:Destroy()
                                end
                                charmCache[plr] = nil
                        ...
                        if not isVis or scanNeeded then
                            isVis = head and espIsVisible(localChar, char)
                            charmVisCache[plr] = isVis
                        end
                        local col = isVis and Config.Charms.VisibleColor or Config.Charms.HiddenColor
                        for part, box in pairs(charmCache[plr]) do
                            if not (part and part.Parent and part:IsDescendantOf(char) and box and box.Parent) then
                                pcall(function()
                                    box:Destroy()
                                end)
                                charmCache[plr][part] = nil
                            end
                        end
                        for part, box in pairs(charmCache[plr]) do
                            ...
                            for _, part in ipairs(char:GetDescendants()) do
                                if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" and part.Transparency < 1 and not charmCache[plr][part] then
                                    ...
                                    if valid then
                                        ...
                                        box.AlwaysOnTop = Config.Charms.AlwaysOnTop
                                        box.ZIndex = 5
                                        box.Parent = charmFolder
                                        charmCache[plr][part] = box
                                    end
                                end
                            end
                        end
...
local GrenadeTrajectory = {
    ...
    LineColor = Color3.fromRGB(220, 220, 255),
    BounceColor = Color3.fromRGB(255, 80, 80),
    EndColor = Color3.fromRGB(255, 60, 60),
    Transparency = 0.15,
    LineWidth = 0.06
}
...
local function getGrenadeNameFallback()
    for _, obj in ipairs(camera:GetChildren()) do
        if obj:IsA("Model") and GRENADE_NAMES[obj.Name] then
            return obj.Name
        end
    end
    return nil
...
local function getInventoryState()
    ...
    if not ok or typeof(fireTable) ~= "table" or typeof(fireTable.Callback) ~= "function" then
        return nil
    end
    local ok2, invCtrl = pcall(debug.getupvalue, fireTable.Callback, 7)
    if not ok2 or typeof(invCtrl) ~= "table" or not invCtrl.getInventorySlot then
        return nil
    end
    local ok3, state = pcall(debug.getupvalue, invCtrl.getInventorySlot, 1)
    if not ok3 or typeof(state) ~= "table" then
        return nil
    end
...
local function getEquippedGrenadeName()
    ...
    if eq then
        ...
        if name and GRENADE_NAMES[name] then
            return name
        end
    end
...
local function detectCollision(from, to, radius, params)
    ...
    for _, off in ipairs(offsets) do
        ...
        if r2 and r2.Distance < bestDist then
            bestDist = r2.Distance
            bestResult = r2
            bestOffset = off
        end
    end
...
local function calcBounce(vel, normal, state, isPlayer, isJumpThrow)
    ...
    if normal.Y > 0.7 and bounced:Dot(bounced) < SLEEP_SQ then
        return Vector3.zero, newState
    end
...
local function simStep(state, config, params, dt)
    ...
    if s.simulationTime >= MAX_SIM_TIME then
        s.isAtRest = true
        return s, "timeout"
    end
    if config.fuse and s.simulationTime >= config.fuse then
        s.isAtRest = true
        return s, "fuse"
    end
    if state.bounceCount >= 20 then
        s.isAtRest = true
    ...
    if col then
        local bVel
        bVel, s = calcBounce(newVel, col.normal, s, col.isPlayer, s.isJumpThrow)
        s.position = col.position + col.normal * 0.05
        s.velocity = bVel
        if config.floorExplode and col.normal.Y > 0.7 then
            if not config.minFuse or s.simulationTime >= config.minFuse then
                s.isAtRest = true
                return s, "floor_impact"
            end
        end
        return s, "bounce"
    end
...
local function createInitialState(origin, dir, throwType, playerVel, rangeScale, timestamp)
    ...
    if combined.Magnitude > cap then
        combined = combined.Unit * cap
    end
...
local function simulateTrajectory(origin, dir, throwType, playerVel, config, params)
    ...
    while not state.isAtRest and iterations < MAX_SEGMENTS do
        ...
        local reason
        state, reason = simStep(state, config, params, STEP)
        table.insert(points, state.position)
        if reason == "bounce" then
            table.insert(bounceIndices, #points)
        end
        if reason == "fuse" or reason == "floor_impact" or reason == "rest" or reason == "timeout" then
            endReason = reason
            break
        end
    end
...
for i = 1, MAX_SEGMENTS do
    ...
    if i > 1 then
        ...
        b.Attachment0 = attachPool[i - 1]
        b.Attachment1 = attachPool[i]
        b.Width0 = GrenadeTrajectory.LineWidth
        b.Width1 = GrenadeTrajectory.LineWidth
        b.FaceCamera = true
        b.LightEmission = 1
        b.LightInfluence = 0
        b.Segments = 1
...
local function showTrajectory(points, bounceIndices)
    local bounceSet = {}
    for _, idx in ipairs(bounceIndices) do
        bounceSet[idx] = true
    end
    local count = math.min(#points, MAX_SEGMENTS)
    for i = 1, count do
        attachPool[i].WorldPosition = points[i]
    end
    for i = 1, count - 1 do
        local beam = beamPool[i]
        beam.Enabled = true
        beam.Width0 = GrenadeTrajectory.LineWidth
        beam.Width1 = GrenadeTrajectory.LineWidth
        if bounceSet[i + 1] then
            beam.Color = ColorSequence.new(GrenadeTrajectory.BounceColor)
        else
            beam.Color = ColorSequence.new(GrenadeTrajectory.LineColor)
        end
        beam.Transparency = NumberSequence.new(GrenadeTrajectory.Transparency)
    end
    for i = count, MAX_SEGMENTS - 1 do
        if beamPool[i] then
...
local function grenadeRenderStep()
    if not GrenadeTrajectory.Enabled then
        hideTrajectory()
        return
    end
    local char = player.Character
...
local function getAllSkins(f)
    local s = {}
    for _, sk in f:GetChildren() do
        table.insert(s, sk.Name)
    end
    return s
end
local function applyWeaponSkin(model)
    if not model or not SkinChangerEnabled or not isAlive() then
        return
    end
    ...
    pcall(function()
        ...
        for _, obj in camera:GetChildren() do
            ...
            if left or right then
                local gf = SkinsFolder:FindFirstChild("Sports Gloves")
                local gs = gf and gf:FindFirstChild(SelectedSkins["Sports Gloves"])
                ...
                if gsrc then
                    for _, side in ipairs({"Left Arm", "Right Arm"}) do
                        ...
                        if arm and src then
                            ...
                            if gloveMesh then
                                local ex = gloveMesh:FindFirstChildOfClass("SurfaceAppearance")
                                if ex then
                                    ex:Destroy()
                                end
                                local c = src:Clone()
...
camera.ChildAdded:Connect(function(obj)
    if not SkinChangerEnabled or not isAlive() then
        return
    end
    task.wait(COOLDOWN)
    applyWeaponSkin(obj)
...
EspSection:Dropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.BoxColor = colorValues[v[1]]
    end
})
EspSection:Dropdown({
    Title = "方框可见颜色",
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.BoxVisibleColor = colorValues[v[1]]
    end
})
EspSection:Dropdown({
    Title = "方框不可见颜色",
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.BoxNotVisibleColor = colorValues[v[1]]
    end
})
EspSection:Toggle({
    Title = "方框填充",
...
EspSection:Dropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.BoxFillColor1 = colorValues[v[1]]
    end
})
EspSection:Dropdown({
    Title = "填充颜色2",
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.BoxFillColor2 = colorValues[v[1]]
    end
})
EspSection:Slider({
    Title = "填充透明度",
    ...
    Callback = function(v)
        Config.ESP.BoxFillTransparency = v
    end
})
EspSection:Slider({
    Title = "渐变速度",
...
EspSection:Dropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.NameColor = colorValues[v[1]]
    end
})
EspSection:Dropdown({
    Title = "名称可见颜色",
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.NameVisibleColor = colorValues[v[1]]
    end
})
EspSection:Dropdown({
    Title = "名称不可见颜色",
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.NameNotVisibleColor = colorValues[v[1]]
    end
})
EspSection:Toggle({
    Title = "生命值条",
...
EspSection:Dropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.HealthBarColor = colorValues[v[1]]
    end
})
EspSection:Toggle({
    Title = "骨骼",
...
EspSection:Slider({
    ...
    Callback = function(v)
        Config.ESP.SkeletonThickness = v
    end
})
EspSection:Dropdown({
    Title = "骨骼颜色",
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.SkeletonColor = colorValues[v[1]]
    end
})
EspSection:Dropdown({
    Title = "骨骼可见颜色",
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.SkeletonVisibleColor = colorValues[v[1]]
    end
})
EspSection:Dropdown({
    Title = "骨骼不可见颜色",
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.SkeletonNotVisibleColor = colorValues[v[1]]
    end
})
EspSection:Toggle({
    Title = "头部圆点",
...
EspSection:Dropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.HeadDotColor = colorValues[v[1]]
    end
})
EspSection:Toggle({
    Title = "高亮",
...
EspSection:Dropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.HighlightFill = colorValues[v[1]]
    end
})
EspSection:Dropdown({
    Title = "高亮轮廓",
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.HighlightOutline = colorValues[v[1]]
    end
})
EspSection:Toggle({
    Title = "距离",
...
EspSection:Dropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.DistanceColor = colorValues[v[1]]
    end
})
EspSection:Toggle({
    Title = "当前武器",
...
EspSection:Dropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.ESP.WeaponColor = colorValues[v[1]]
    end
})
local WorldEspSection = VisualTab:Section({
    Title = "世界 ESP"
})
WorldEspSection:Toggle({
    ...
    Value = true,
    Callback = function(v)
        Config.WorldESP.DroppedWeapons.Enabled = v
    end
})
WorldEspSection:Toggle({
    Title = "掉落-方框",
    Value = true,
    Callback = function(v)
        Config.WorldESP.DroppedWeapons.Box = v
    end
})
WorldEspSection:Toggle({
    Title = "掉落-高亮",
    Value = true,
    Callback = function(v)
        Config.WorldESP.DroppedWeapons.Highlight = v
    end
})
WorldEspSection:Toggle({
    Title = "掉落-名称",
    Value = true,
    Callback = function(v)
        Config.WorldESP.DroppedWeapons.Name = v
    end
})
WorldEspSection:Toggle({
    Title = "C4炸弹",
    Value = true,
    Callback = function(v)
        Config.WorldESP.Bomb.Enabled = v
    end
})
WorldEspSection:Toggle({
    Title = "炸弹-方框",
...
WorldEspSection:Toggle({
    ...
    Value = true,
    Callback = function(v)
        Config.WorldESP.Bomb.Highlight = v
    end
})
WorldEspSection:Toggle({
    Title = "炸弹-名称",
...
WorldEspSection:Toggle({
    ...
    Value = true,
    Callback = function(v)
        Config.WorldESP.Molotovs.Enabled = v
    end
})
WorldEspSection:Toggle({
    Title = "火焰-高亮",
    Value = true,
    Callback = function(v)
        Config.WorldESP.Molotovs.Highlight = v
    end
})
WorldEspSection:Toggle({
    Title = "烟雾 ESP",
    Value = true,
    Callback = function(v)
        Config.WorldESP.Smokes.Enabled = v
    end
})
WorldEspSection:Toggle({
    Title = "烟雾-高亮",
    Value = true,
    Callback = function(v)
        Config.WorldESP.Smokes.Highlight = v
    end
})
local CharmSection = VisualTab:Section({
    Title = "Charms"
})
CharmSection:Toggle({
    Title = "启用",
...
CharmSection:Dropdown({
    ...
    Multi = false,
    Callback = function(v)
        Config.Charms.VisibleColor = colorValues[v[1]]
    end
})
CharmSection:Dropdown({
    Title = "不可见颜色",
    ...
    Multi = false,
    Callback = function(v)
        Config.Charms.HiddenColor = colorValues[v[1]]
    end
})
CharmSection:Slider({
    Title = "透明度",
...
CharmSection:Toggle({
    ...
    Value = true,
    Callback = function(v)
        Config.Charms.AlwaysOnTop = v
    end
})
local TrajectorySection = VisualTab:Section({
    Title = "投掷物轨迹"
})
TrajectorySection:Toggle({
    Title = "启用投掷物轨迹",
    Value = false,
    Callback = function(v)
        GrenadeTrajectory.Enabled = v
        if not v then
            hideTrajectory()
        end
    end
})
TrajectorySection:Dropdown({
    Title = "显示模式",
    Values = {"自动", "热键"},
    Value = {"自动"},
    Multi = false,
    Callback = function(v)
        GrenadeTrajectory.Mode = v[1]
    end
})
TrajectorySection:Dropdown({
    Title = "热键",
    ...
    Multi = false,
    Callback = function(v)
        GrenadeTrajectory.Hotkey = hotkeyMap[v[1]] or Enum.UserInputType.MouseButton2
    end
})
TrajectorySection:Dropdown({
    Title = "轨迹线条颜色",
    ...
    Multi = false,
    Callback = function(v)
        GrenadeTrajectory.LineColor = colorValues[v[1]]
    end
})
TrajectorySection:Dropdown({
    Title = "弹跳点颜色",
    ...
    Multi = false,
    Callback = function(v)
        GrenadeTrajectory.BounceColor = colorValues[v[1]]
    end
})
TrajectorySection:Dropdown({
    Title = "终点颜色",
    ...
    Multi = false,
    Callback = function(v)
        GrenadeTrajectory.EndColor = colorValues[v[1]]
        endSphere.Color = colorValues[v[1]]
    end
})
TrajectorySection:Slider({
    Title = "线条透明度",
    Step = 0.01,
    Value = {
        Min = 0,
    ...
    Callback = function(v)
        GrenadeTrajectory.Transparency = v
    end
})
TrajectorySection:Slider({
    Title = "线条粗细",
...
local SkinSection = SkinTab:Section({
    Title = "皮肤修改器"
})
SkinSection:Toggle({
    Title = "启用皮肤修改器",
...
SkinSection:Button({
    Title = "随机所有皮肤",
    Callback = function()
        for wn, ol in pairs(SkinOptions) do
            if #ol > 0 then
                local rs = ol[math.random(1, #ol)]
                if DropdownObjects[wn] then
                    for _, dd in ipairs(DropdownObjects[wn]) do
                        dd:SetValue(rs)
                    end
                end
            end
...
local KnifeSection = SkinTab:Section({
    Title = "自定义刀子"
})
...
local HeavySwingAnim = nil
local Swing1Anim = nil
local Swing2Anim = nil
...
local function disableCollisions(m)
    for _, p in m:GetDescendants() do
        cleanPart(p)
    end
end
local function hideOriginalKnife(k)
    for _, p in k:GetDescendants() do
        if p:IsA("BasePart") or p:IsA("MeshPart") or p:IsA("Texture") then
            p.Transparency = 1
        end
    end
end
local function playSound(folder, name)
    local ws = RS.Sounds:FindFirstChild(selectedKnife)
    if not ws then
        return
    end
    local s = ws:WaitForChild(folder):WaitForChild(name):Clone()
    s.Parent = camera
    s:Play()
...
local function attachAsset(folder, armPart, assetModel, finalName, offset)
    local arm = vm:FindFirstChild(armPart)
    if not arm then
        return
    end
    local mesh = folder:WaitForChild(assetModel):Clone()
    cleanPart(mesh)
    mesh.Name = finalName
    mesh.Parent = arm
...
local function handleAction(actionName, inputState, inputObject)
    ...
    elseif actionName == ACTION_ATTACK then
        local ct = os.clock()
        if (equipAnim and equipAnim.IsPlaying) or (ct - lastAttackTime < ATTACK_COOLDOWN) then
            return Enum.ContextActionResult.Pass
        end
        lastAttackTime = ct
        if inspecting then
        ...
        chosen:Play()
        local s = playSound(sf, "1")
        if s then
            s.Volume = 5
        end
        chosen.Stopped:Once(function()
            swinging = false
        end)
    end
    return Enum.ContextActionResult.Pass
...
local function spawnViewmodel(knife)
    if spawned or not scriptRunning then
        return
    end
    ...
    if myModel.Parent.Name == "Terrorists" then
        ...
        local ctg = RS.Assets.Weapons:WaitForChild("CT Glove")
...
RunService.RenderStepped:Connect(function()
    if not scriptRunning or not vm or not vm.PrimaryPart then
        return
    end
...
local SkinRightSection = SkinTab:Section({
    Title = "武器皮肤"
})
    ...
    local dp = group:Dropdown({
        ...
        Callback = function(opt)
            ...
            if DropdownObjects[weaponName] then
                for _, other in ipairs(DropdownObjects[weaponName]) do
                    if other.Value ~= opt[1] then
                        other:SetValue(opt[1])
                    end
                end
            end
            for _, obj in camera:GetChildren() do
...
SkinRightSection:Divider()
SkinRightSection:Paragraph({
    Title = "刀具皮肤",
    Desc = ""
})
for name in pairs(KNIVES) do
    CreateSkinDropdown(name, SkinRightSection)
end
SkinRightSection:Divider()
SkinRightSection:Paragraph({
    Title = "手套",
...
for _, folder in SkinsFolder:GetChildren() do
    ...
    if not IgnoreFolders[n] and not KNIVES[n] and not GLOVES[n] and not CT_ONLY[n] and not SHARED[n] then
        CreateSkinDropdown(n, SkinRightSection)
    end
end
...
local UISection = UISettingsTab:Section({
    Title = "菜单"
})
UISection:Button({
    Title = "卸载脚本",
