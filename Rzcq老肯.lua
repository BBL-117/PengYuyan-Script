...
local AutoSettings = {
    AutoSwing = false, AutoSell = false, AutoR = false, AutoS = false,
    AutoB = false, AutoC = false, AutoE = false, AutoCr = false,
    AutoTa = false, AutoBo = false, AutoBo1 = false, AutoBo2 = false
}
for k, v in pairs(AutoSettings) do getgenv()[k] = v end
local function teleportTo(placeCFrame)
    local plyr = game.Players.LocalPlayer
    if plyr.Character then plyr.Character.HumanoidRootPart.CFrame = placeCFrame end
end
local AutoFunctions = {
    doBo = function()
        spawn(function()
    ...
    doE = function()
        spawn(function()
            while getgenv().AutoE == true do
                if not getgenv() then break end
                local elements = {"Inferno","Frost","Lightning","Electral Chaos","Shadow Charge","Masterful Wrath","Shadowfire","Eternity Storm","Blazing Entity"}
...
local PlayerList = {}
for _, p in pairs(Plr:GetPlayers()) do table.insert(PlayerList, p.Name) end
Plr.PlayerAdded:Connect(function(a)
    if not table.find(PlayerList, a.Name) then table.insert(PlayerList, a.Name) end
end)
Plr.PlayerRemoving:Connect(function(a)
    local idx = table.find(PlayerList, a.Name)
    if idx then table.remove(PlayerList, idx) end
end)
local killTaskHandle
local function startKill()
    if killTaskHandle then task.cancel(killTaskHandle) end
            ...
            for _, playerName in pairs(getgenv().KillPlayers) do
                ...
                if player and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                    LP.Character.HumanoidRootPart.CFrame = CFrame.new(
                        player.Character.HumanoidRootPart.Position + Vector3.new(math.sin(tick() * SpinSpeed * math.pi) * Radius, Height, math.cos(tick() * SpinSpeed * math.pi) * Radius),
...
local function startMassKill()
    if massKillTaskHandle then task.cancel(massKillTaskHandle) end
...
game:GetService("RunService").Heartbeat:Connect(function()
    if getgenv().KillEnabled and not killTaskHandle then
        startKill()
    elseif not getgenv().KillEnabled and killTaskHandle then
        task.cancel(killTaskHandle)
        killTaskHandle = nil
        workspace.Gravity = 196.2
    end
    if getgenv().MassKillEnabled and not massKillTaskHandle then
        startMassKill()
    elseif not getgenv().MassKillEnabled and massKillTaskHandle then
        task.cancel(massKillTaskHandle)
        massKillTaskHandle = nil
        workspace.Gravity = 196.2
    end
end)
function setMultiJump(count)
    game.Players.LocalPlayer.multiJumpCount.Value = tonumber(count) or 1
end
function unlockAllIslands()
    local positions = {
        CFrame.new(26, 766, -114), CFrame.new(247, 2013, 347), CFrame.new(162, 4047, 13),
        CFrame.new(200, 5656, 13), CFrame.new(200, 9284, 13), CFrame.new(200, 13679, 13),
        CFrame.new(200, 17686, 13), CFrame.new(200, 24069, 13), CFrame.new(197, 28256, 7),
        CFrame.new(197, 33206, 7), CFrame.new(197, 39317, 7), CFrame.new(197, 46010, 7),
        CFrame.new(197, 52607, 7), CFrame.new(197, 59594, 7), CFrame.new(197, 66668, 7),
        CFrame.new(197, 70270, 7), CFrame.new(197, 74442, 7), CFrame.new(197, 79746, 7),
        CFrame.new(197, 83198, 7), CFrame.new(197, 91245, 7)
    }
...
function autoCollectChests()
    spawn(function()
        local playerHead = game.Players.LocalPlayer.Character.Head
        local chests = {"ultraNinjitsuChest","mythicalChest","goldenChest","enchantedChest","magmaChest","legendsChest","saharaChest","eternalChest","ancientChest","midnightShadowChest","wonderChest","goldenZenChest","skystormMastersChest","chaosLegendsChest","soulFusionChest"}
        while wait() do
...
local selectedEgg = ""
function startAutoEgg(eggName)
    selectedEgg = eggName
    spawn(function()
        while getgenv().openegg do
            wait()
            local Event = game:GetService("ReplicatedStorage").rEvents.openCrystalRemote
            Event:InvokeServer("openCrystal", selectedEgg)
        end
    end)
end
local function startAll()
    if getgenv().AutoSwing then AutoFunctions.doSwing() end
    if getgenv().AutoSell then AutoFunctions.doSell() end
    if getgenv().AutoR then AutoFunctions.doR() end
    if getgenv().AutoS then AutoFunctions.doS() end
    if getgenv().AutoB then AutoFunctions.doB() end
    if getgenv().AutoC then AutoFunctions.doC() end
    if getgenv().AutoBo then AutoFunctions.doBo() end
    if getgenv().AutoBo1 then AutoFunctions.doBo1() end
    if getgenv().AutoBo2 then AutoFunctions.doBo2() end
end
startAll()
...
A:Button({
    Title = "初始化第一步",
    Callback = function()
        game:GetService("ReplicatedStorage"):WaitForChild("rEvents"):WaitForChild("zenMasterEvent"):FireServer("convertGems", -)
    end
})
A:Button({
    Title = "初始化第二步",
...
F:Toggle({
    ...
    Callback = function(Value)
        ...
        if Value then AutoFunctions.doBo() end
    end
})
F:Toggle({
    Title = "永恒Boss",
    ...
    Callback = function(Value)
        ...
        if Value then AutoFunctions.doBo1() end
    end
})
F:Toggle({
    Title = "岩浆Boss",
    ...
    Callback = function(Value)
        ...
        if Value then AutoFunctions.doBo2() end
    end
})
