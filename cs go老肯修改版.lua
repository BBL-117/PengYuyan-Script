...
ls:SetOutOfScope(false)
ls:SetMessageMode(Enum.LogMessageMode.None)
end)
pcall(function()
for _,v in pairs(getgc(true))do
if type(v)=="table"and rawget(v,"Name")and rawget(v,"Source")then
if tostring(v.Name):find("Aero")then v.Name="";v.Source=""end
end
end
end)
pcall(function()
...
hookfunction(old,function()return nil end)
end)
pcall(function()
for k,v in pairs(_G)do
if type(v)=="function"and tostring(k):lower():find("detect")then _G[k]=nil end
end
end)
pcall(function()
local rmt=game:GetService("ReplicatedStorage"):FindFirstChild("AntiCheat")
if rmt then rmt:Destroy()end
end)
pcall(function()
local h=debug.getupvalue or function()end
if type(h)=="function"then
for i=1,10 do pcall(function()h(function()end,i)end)end
end
end)
pcall(function()
local s=string.dump
string.dump=function(f,strip)if strip then return s(f,true)end return s(f)end
end)
pcall(function()
local old=game.GetService
game.GetService=function(self,name)
if name=="LogService"or name=="ScriptInformationProvider"then return nil end
return old(self,name)
end
end)
pcall(function()
for _,v in pairs(getgc(true))do
if type(v)=="table"and rawget(v,"Script")and type(rawget(v,"Script"))=="table"then
if rawget(v,"Script"):find("Aero")then rawget(v,"Script")=""end
end
end
end)
end
...
local Window=Library:CreateWindow({Title=winTitle,Footer=winFooter,NotifySide="Right",ShowCustomCursor=true})
local Tabs={Combat=Window:AddTab("战斗","crosshair"),Skins=Window:AddTab("皮肤","swords"),Visuals=Window:AddTab("视觉","eye"),["UI Settings"]=Window:AddTab("界面设置","settings")}
...
local hotkeyMap={["LeftAlt"]=Enum.KeyCode.LeftAlt,["LeftShift"]=Enum.KeyCode.LeftShift,["RightShift"]=Enum.KeyCode.RightShift,["LeftControl"]=Enum.KeyCode.LeftControl,["RightControl"]=Enum.KeyCode.RightControl,["Q"]=Enum.KeyCode.Q,["E"]=Enum.KeyCode.E,["R"]=Enum.KeyCode.R,["F"]=Enum.KeyCode.F,["X"]=Enum.KeyCode.X,["C"]=Enum.KeyCode.C,["V"]=Enum.KeyCode.V,["鼠标右键"]=Enum.UserInputType.MouseButton2,["鼠标左键"]=Enum.UserInputType.MouseButton1,["鼠标中键"]=Enum.UserInputType.MouseButton3}
...
mp=Vector2.new(camera.ViewportSize.X/2,camera.ViewportSize.Y/2)
else
...
if UserInputService.TouchEnabled then FOVCircle.Position=Vector2.new(camera.ViewportSize.X/2,camera.ViewportSize.Y/2)end
FOVCircle.Radius=VisualAimbot.Radius;FOVCircle.Visible=true
...
local mode=TriggerBot.Mode
if UserInputService.TouchEnabled then mode="自动"end
if mode=="自动"then shoot=true elseif mode=="热键"then shoot=TriggerBot.KeyHeld end
end
...
local knives={["Karambit"]={Offset=CFrame.new(0,-1.5,1.5)},["Butterfly Knife"]={Offset=CFrame.new(0,-1.5,1.5)},["M9 Bayonet"]={Offset=CFrame.new(0,-1.5,1)},["Flip Knife"]={Offset=CFrame.new(0,-1.5,1.25)},["Gut Knife"]={Offset=CFrame.new(0,-1.5,0.5)},["Stiletto Knife"]={Offset=CFrame.new(0,-1.5,1.25)},["Skeleton Knife"]={Offset=CFrame.new(0,-1.5,1.25)}}
local vm,animator
...
for _,obj in camera:GetChildren()do obj:SetAttribute("SkinApplied",nil)applyWeaponSkin(obj)end end
})
...
local colorValues={["白色"]=Color3.fromRGB(255,255,255),["红色"]=Color3.fromRGB(255,0,0),["绿色"]=Color3.fromRGB(0,255,0),["蓝色"]=Color3.fromRGB(0,0,255),["黄色"]=Color3.fromRGB(255,255,0),["青色"]=Color3.fromRGB(0,255,255),["紫色"]=Color3.fromRGB(128,0,128),["橙色"]=Color3.fromRGB(255,165,0),["黑色"]=Color3.fromRGB(0,0,0)}
local Config={ESP={Enabled=true,TeamCheck=true,VisibilityCheck=true,MaxDistance=2000,Box=true,BoxThickness=1,BoxOutline=false,BoxFill=false,BoxFillColor1=Color3.fromRGB(255,0,0),BoxFillColor2=Color3.fromRGB(0,0,255),BoxFillTransparency=0.8,BoxFillFadeSpeed=3,Name=true,NameSize=13,Health=true,HealthBarCustom=false,HealthBarColor=Color3.fromRGB(0,255,0),Skeleton=false,SkeletonThickness=2,HeadDot=false,Highlight=true,Distance=true,CurrentWeapon=false,BoxColor=Color3.fromRGB(255,255,255),BoxVisibleColor=Color3.fromRGB(0,255,0),BoxNotVisibleColor=Color3.fromRGB(255,0,0),NameColor=Color3.fromRGB(255,255,255),NameVisibleColor=Color3.fromRGB(0,255,0),NameNotVisibleColor=Color3.fromRGB(255,0,0),SkeletonColor=Color3.fromRGB(255,255,255),SkeletonVisibleColor=Color3.fromRGB(0,255,0),SkeletonNotVisibleColor=Color3.fromRGB(255,0,0),HeadDotColor=Color3.fromRGB(255,255,255),HeadDotVisibleColor=Color3.fromRGB(0,255,0),HeadDotNotVisibleColor=Color3.fromRGB(255,0,0),HighlightFill=Color3.fromRGB(255,0,0),HighlightOutline=Color3.fromRGB(255,255,255),HighlightVisibleFill=Color3.fromRGB(0,255,0),HighlightHiddenFill=Color3.fromRGB(255,0,0),DistanceColor=Color3.fromRGB(255,255,255),WeaponColor=Color3.fromRGB(255,255,255)},WorldESP={DroppedWeapons={Enabled=true,Box=true,Highlight=true,Name=true,Color=Color3.fromRGB(255,255,255)},Bomb={Enabled=true,Box=true,Highlight=true,Name=true,Color=Color3.fromRGB(255,0,0)},Molotovs={Enabled=true,Highlight=true,Color=Color3.fromRGB(255,165,0)},Smokes={Enabled=true,Highlight=true,Color=Color3.fromRGB(200,200,200)}},Charms={Enabled=true,TeamCheck=true,VisibleColor=Color3.fromRGB(255,0,0),HiddenColor=Color3.fromRGB(255,255,255),Transparency=0.5,AlwaysOnTop=true}}
...
group:AddDropdown("Color_"..name,{Text=name,Values=colorPresets,Default=curName,Tooltip=tooltip or"",Callback=function(Value)callback(colorValues[Value])end})
end
...
CreateVisualsUI()
local GrenadeTrajectory={Enabled=false,Mode="自动",Hotkey=Enum.UserInputType.MouseButton2,LineColor=Color3.fromRGB(220,220,255),BounceColor=Color3.fromRGB(255,80,80),EndColor=Color3.fromRGB(255,60,60),Transparency=0.15,LineWidth=0.06}
local MAX_SEGMENTS=180
local STEP=0.0078125
local MAX_SIM_TIME=10
local GRENADE_NAMES={["HE Grenade"]={radius=0.15,fuse=1.6},["Flashbang"]={radius=0.15,fuse=1.5},["Smoke Grenade"]={radius=0.15,fuse=3.0},["Molotov"]={radius=0.15,fuse=5.0,minFuse=0.3,floorExplode=true},["Incendiary Grenade"]={radius=0.15,fuse=5.0,minFuse=0.3,floorExplode=true},["Decoy Grenade"]={radius=0.15,fuse=15.0}}
local function getGrenadeNameFallback()for _,obj in ipairs(camera:GetChildren())do if obj:IsA("Model")and GRENADE_NAMES[obj.Name]then return obj.Name end end return nil end
local _inventoryState=nil
local function getInventoryState()
if _inventoryState then return _inventoryState end
local fireMod
for _,mod in ipairs(getloadedmodules())do
if mod:GetFullName():find("Controllers.InputController.Actions.Fire",1,true)then fireMod=mod break end
end
...
_inventoryState=state return state
end
local function getCurrentEquippedSafe()local state=getInventoryState()if not state then return nil end return state.CurrentEquipped end
local function getEquippedGrenadeName()local eq=getCurrentEquippedSafe()if eq then local name=eq.weapon or eq.Name or eq.name or eq.identifier if name and GRENADE_NAMES[name]then return name end end return getGrenadeNameFallback()end
local GRAVITY_VEC=Vector3.new(0,-23.833334,0)
local STOP_EPS=0.0076388888888888895
local SLEEP_SQ=2.3341049382716053
local function clipVelocity(vel,normal,overbounce)local d=vel:Dot(normal)*overbounce local out=vel-normal*d local x=math.abs(out.X)<STOP_EPS and 0 or out.X local y=math.abs(out.Y)<STOP_EPS and 0 or out.Y local z=math.abs(out.Z)<STOP_EPS and 0 or out.Z return Vector3.new(x,y,z)end
local function integrate(pos,vel,dt,grounded)if grounded then return pos+vel*dt,vel end local vy=vel.Y-dt*23.833334 local avgY=(vel.Y+vy)/2 local dp=Vector3.new(vel.X*dt,avgY*dt,vel.Z*dt)return pos+dp,Vector3.new(vel.X,vy,vel.Z)end
local function detectCollision(from,to,radius,params)local dir=to-from local mag=dir.Magnitude if mag<0.001 then return nil end local pad=radius*0.01 local offsets={Vector3.new(pad,0,0),Vector3.new(-pad,0,0),Vector3.new(0,pad,0),Vector3.new(0,-pad,0),Vector3.new(0,0,pad),Vector3.new(0,0,-pad)}local bestDist=math.huge local bestResult=nil local bestOffset=Vector3.zero local r=workspace:Raycast(from,dir,params)if r and r.Distance<bestDist then bestDist=r.Distance bestResult=r bestOffset=Vector3.zero end for _,off in ipairs(offsets)do local r2=workspace:Raycast(from+off,dir,params)if r2 and r2.Distance<bestDist then bestDist=r2.Distance bestResult=r2 bestOffset=off end end if not bestResult then return nil end local hitPos=bestResult.Position-bestOffset local dist=(hitPos-from).Magnitude if mag+pad+0.1<dist then return nil end local parent=bestResult.Instance.Parent local isPlayer=parent and parent:FindFirstChildOfClass("Humanoid")~=nil return{position=hitPos,normal=bestResult.Normal,distance=dist,isPlayer=isPlayer}end
local function checkGrounded(pos,params)local r=workspace:Raycast(pos,Vector3.new(0,-0.2,0),params)if r then return true,r.Normal end return false,nil end
local function calcBounce(vel,normal,state,isPlayer,isJumpThrow)local elast=(isJumpThrow and 0.32 or 0.4)*(isPlayer and 0.3 or 1)elast=math.clamp(elast,0,0.9)local bounced=clipVelocity(vel,normal,2)*elast local newState=table.clone(state)newState.bounceCount=state.bounceCount+1 newState.hasTouched=true if normal.Y>0.7 and bounced:Dot(bounced)<SLEEP_SQ then return Vector3.zero,newState end return bounced,newState end
local function simStep(state,config,params,dt)local s=table.clone(state)s.simulationTime=state.simulationTime+dt if s.simulationTime>=MAX_SIM_TIME then s.isAtRest=true return s,"timeout"end if config.fuse and s.simulationTime>=config.fuse then s.isAtRest=true return s,"fuse"end if state.bounceCount>=20 then s.isAtRest=true s.velocity=Vector3.zero return s,"rest"end local oldPos=s.position local newPos,newVel=integrate(s.position,s.velocity,dt,s.isGrounded)local col=detectCollision(oldPos,newPos,config.radius,params)if col then local bVel bVel,s=calcBounce(newVel,col.normal,s,col.isPlayer,s.isJumpThrow)s.position=col.position+col.normal*0.05 s.velocity=bVel if config.floorExplode and col.normal.Y>0.7 then if not config.minFuse or s.simulationTime>=config.minFuse then s.isAtRest=true return s,"floor_impact"end end return s,"bounce"end s.position=newPos s.velocity=newVel local grounded=checkGrounded(s.position,params)s.isGrounded=grounded if grounded and s.hasTouched and s.velocity:Dot(s.velocity)<SLEEP_SQ then if not config.fuse then s.isAtRest=true s.velocity=Vector3.zero return s,"rest"end end return s,nil end
local function calcThrowParams(rootPos,lookVec,throwType,pitchScale)local isNear=(throwType=="Near")local upBias=(isNear and 0.04 or 0.06)*math.clamp(pitchScale or 1,0.8,1.2)local fwdOff=isNear and(1.35*0.55)or 1.35 local hOff=isNear and(2.4*0.8)or(2.4+0.1)if isNear then upBias=upBias+0.08 end local dir=(lookVec+Vector3.new(0,upBias,0)).Unit local flat=Vector3.new(lookVec.X,0,lookVec.Z)if flat.Magnitude<0.01 then flat=Vector3.new(0,0,-1)end flat=flat.Unit local origin=rootPos+flat*fwdOff+Vector3.new(0,hOff,0)return origin,dir end
local function createInitialState(origin,dir,throwType,playerVel,rangeScale,timestamp)local throwPower=((throwType=="Far"and 1 or 0)*0.7+0.3)*57.29166*(rangeScale or 1)*0.58 local isJump=playerVel.Y>5 local dampening=1 local vertAdd=isJump and Vector3.new(0,20,0)or Vector3.new(0,playerVel.Y*2*0.58,0)local throwDir=isJump and Vector3.new(dir.X*dampening,dir.Y,dir.Z*dampening).Unit or dir local baseVel=throwDir*throwPower+vertAdd local scaledDir=Vector3.new(dir.X*(isJump and dampening or 1),dir.Y,dir.Z*(isJump and dampening or 1)).Unit local upVel=scaledDir*throwPower*0.15+Vector3.new(0,(rangeScale or 1)*6.5*0.58,0)local combined=baseVel+upVel local cap=isJump and((dir.Y-0.4)*20+62)or 50 if combined.Magnitude>cap then combined=combined.Unit*cap end local pvelScale=isJump and dampening or 1 combined=combined+Vector3.new(playerVel.X,0,playerVel.Z)*1.5*pvelScale return{position=origin,velocity=combined,simulationTime=0,bounceCount=0,isGrounded=false,isAtRest=false,hasTouched=false,accumulatedTime=0,isJumpThrow=isJump}end
local function simulateTrajectory(origin,dir,throwType,playerVel,config,params)local state=createInitialState(origin,dir,throwType,playerVel,1,tick())local points={state.position}local bounceIndices={}local endReason=nil local iterations=0 while not state.isAtRest and iterations<MAX_SEGMENTS do iterations=iterations+1 local reason state,reason=simStep(state,config,params,STEP)table.insert(points,state.position)if reason=="bounce"then table.insert(bounceIndices,#points)end if reason=="fuse"or reason=="floor_impact"or reason=="rest"or reason=="timeout"then endReason=reason break end end return points,bounceIndices,endReason end
local holder=Instance.new("Part")holder.Name="dvfx"holder.Size=Vector3.new(0.05,0.05,0.05)holder.Transparency=1 holder.Anchored=true holder.CanCollide=false holder.CanQuery=false holder.CanTouch=false holder.CastShadow=false holder.CFrame=CFrame.identity holder.Parent=camera
local attachPool={}local beamPool={}
for i=1,MAX_SEGMENTS do
local a=Instance.new("Attachment")a.Parent=holder attachPool[i]=a
if i>1 then
local b=Instance.new("Beam")b.Attachment0=attachPool[i-1]b.Attachment1=attachPool[i]b.Width0=GrenadeTrajectory.LineWidth b.Width1=GrenadeTrajectory.LineWidth b.FaceCamera=true b.LightEmission=1 b.LightInfluence=0 b.Segments=1 b.TextureMode=Enum.TextureMode.Stretch b.Color=ColorSequence.new(GrenadeTrajectory.LineColor)b.Transparency=NumberSequence.new(GrenadeTrajectory.Transparency)b.Enabled=false b.Parent=holder beamPool[i-1]=b
end
end
local endSphere=Instance.new("Part")endSphere.Name="ep"endSphere.Shape=Enum.PartType.Ball endSphere.Size=Vector3.new(0.6,0.6,0.6)endSphere.Material=Enum.Material.Neon endSphere.Color=GrenadeTrajectory.EndColor endSphere.Anchored=true endSphere.CanCollide=false endSphere.CanQuery=false endSphere.CanTouch=false endSphere.CastShadow=false endSphere.Transparency=1 endSphere.Parent=camera
local function showTrajectory(points,bounceIndices)local bounceSet={}for _,idx in ipairs(bounceIndices)do bounceSet[idx]=true end local count=math.min(#points,MAX_SEGMENTS)for i=1,count do attachPool[i].WorldPosition=points[i]end for i=1,count-1 do local beam=beamPool[i]beam.Enabled=true beam.Width0=GrenadeTrajectory.LineWidth beam.Width1=GrenadeTrajectory.LineWidth if bounceSet[i+1]then beam.Color=ColorSequence.new(GrenadeTrajectory.BounceColor)else beam.Color=ColorSequence.new(GrenadeTrajectory.LineColor)end beam.Transparency=NumberSequence.new(GrenadeTrajectory.Transparency)end for i=count,MAX_SEGMENTS-1 do if beamPool[i]then beamPool[i].Enabled=false end end local endPos=points[count]endSphere.CFrame=CFrame.new(endPos)endSphere.Color=GrenadeTrajectory.EndColor endSphere.Transparency=GrenadeTrajectory.Transparency+0.15 end
local function hideTrajectory()for i=1,MAX_SEGMENTS-1 do if beamPool[i]then beamPool[i].Enabled=false end end endSphere.Transparency=1 end
...
local char=player.Character if not char then hideTrajectory()return end
local root=char:FindFirstChild("HumanoidRootPart")if not root then hideTrajectory()return end
local grenadeName=getEquippedGrenadeName()if not grenadeName then hideTrajectory()return end
local shouldShow=false
if GrenadeTrajectory.Mode=="自动"then shouldShow=true
elseif GrenadeTrajectory.Mode=="热键"then
local key=GrenadeTrajectory.Hotkey
if typeof(key)=="EnumItem"then
if key.EnumType==Enum.UserInputType then shouldShow=UserInputService:IsMouseButtonPressed(key)
elseif key.EnumType==Enum.KeyCode then shouldShow=UserInputService:IsKeyDown(key)end
end
end
